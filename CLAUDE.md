...
The backend app deploys via `docker: image:` pointing at the image `.github/workflows/native-image.yaml` publishes to GHCR (see Build above) — Cloud Foundry pulls the already-built native-image container rather than building from source.

The frontend app deploys via the `web_servers_cnb_beta` buildpack (confirmed available on this foundation via `cf buildpacks`, alongside the older/stable `staticfile_buildpack` this project used at first — switched once `web_servers_cnb_beta`'s build-from-source support was verified working end-to-end), pushed as source (`path: frontend`, `frontend/.cfignore` keeps `node_modules`/`dist` out of the upload) rather than a pre-built `dist/`. `BP_NODE_RUN_SCRIPTS: build` has the buildpack run this project's own `npm run build` during staging; `BP_WEB_SERVER_ROOT: dist` serves Vite's output over nginx; `BP_WEB_SERVER_ENABLE_PUSH_STATE: "true"` is this buildpack's equivalent of `staticfile_buildpack`'s old `Staticfile` `pushstate: enabled` config (removed - no longer used), keeping React Router's client-side routes working on a direct load/refresh. This is also why `VITE_API_BASE_URL` lives in the frontend app's own `env:` block rather than needing a separate manual build step: Cloud Foundry adds the manifest's `env:` block to the *staging* container (where `npm run build` actually runs), not just the runtime container - confirmed against CF's own manifest-attributes docs before relying on it, since this is a build-time-vs-runtime distinction that's easy to get wrong.
