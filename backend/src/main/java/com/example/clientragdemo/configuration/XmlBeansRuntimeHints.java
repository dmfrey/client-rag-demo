...
// registration, since GraalVM's static analysis never reaches these classes (nothing in this app's
// own code references them by name) and native-image never puts them in the image, so the lookup
// fails as "class not found" rather than "reflection not registered" and the resulting linker
// simply has no path to the xml:space/xml:lang types. Surfaced as a real .docx failing with
...
// multi-thousand-class registrations in PoiRuntimeHints/OpenAiRuntimeHints.
