...
@Service
class UnarchiveChatSessionService implements UnarchiveChatSessionUseCase {

    private final LoadChatSessionByIdPort loadChatSessionByIdPort;
    private final SaveChatSessionPort saveChatSessionPort;

    UnarchiveChatSessionService(LoadChatSessionByIdPort loadChatSessionByIdPort, SaveChatSessionPort saveChatSessionPort) {
        this.loadChatSessionByIdPort = loadChatSessionByIdPort;
        this.saveChatSessionPort = saveChatSessionPort;
    }

    @Override
    public ChatSession execute(UnarchiveChatSessionCommand command) {
        ...
        return saveChatSessionPort.save(new ChatSession(
                session.id(), session.ownerUsername(), session.title(), false, session.createdAt(), Instant.now()));
    }
}
